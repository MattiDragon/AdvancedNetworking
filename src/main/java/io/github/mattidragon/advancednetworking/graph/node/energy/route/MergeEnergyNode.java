...
public class MergeEnergyNode extends SingleSliderNode {
    private int count = 2;

    public MergeEnergyNode(Graph graph) {
        super(ModNodeTypes.MERGE_ENERGY, List.of(), graph);
    }
    ...
    public Connector<?>[] getInputs() {
        ...
        for (int i = 0; i < connectors.length; i++) {
            connectors[i] = ModDataTypes.ENERGY_STREAM.makeRequiredInput(String.valueOf(i), this);
        }
    ...
    public Connector<?>[] getOutputs() {
        return new Connector[] { ModDataTypes.ENERGY_STREAM.makeOptionalOutput("combined", this) };
    }
