...
public class FilterFluidNode extends Node {
    ...
    public List<Text> validate() {
        ...
        if (!fluidId.isBlank()) {
            ...
            if (id == null)
                ...
                list.add(Text.translatable("node.advanced_networking.filter_fluid.unknown_item", id));
        }
    ...
    protected Either<DataValue<?>[], Text> process(DataValue<?>[] inputs, ContextProvider context) {
        var item = fluidId.isBlank() ? null : Registries.FLUID.get(new Identifier(fluidId));
        ...
        stream.transform(new FluidTransformer.Filter(buildFilter(item, tag, path, whitelist)));
